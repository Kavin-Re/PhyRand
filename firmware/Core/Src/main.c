...
/* USER CODE BEGIN PV */
uint8_t bit_buffer[2];     // Von Neumann pair buffer
...
int main(void)
{
  ...
  while (1)
  {
      ...
      if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
      {
          ...
          if (current_pair == 0) {
              bit_buffer[0] = raw_bit;
              current_pair = 1;
          } else {
              bit_buffer[1] = raw_bit;
              current_pair = 0;
...
/**
  * @brief System Clock Configuration (Standard 84MHz for Nucleo-F401RE)
  */
void SystemClock_Config(void)
{
